...
impl Font {
    ...
    fn glyph_count(&self) -> usize {
        match self.psf_version() {
            ...
            PsfVersion::Psf1 => if self.psf[2] & 0x01 != 0 {512} else {256},
            _ => panic!("Invalid PSF file")
        }
    }
    ...
    fn generate_glyph_map_psf2(&self) -> BTreeMap<char, usize> {
        ...
        let mut map_char = |c: u32, g: usize| {
            unsafe { map.insert(char::from_u32_unchecked(c), g); }
        };
        while offset < self.psf.len() && gid < self.glyph_count() {
            ...
            match state {
                ...
                State::OneByteOutofTwo(b1) => match byte {
                    0b1000_0000..=0b1011_1111 => {state = State::ZeroBytes; map_char(((b1&0x1F) as u32)<<6|((byte&0x3F) as u32), gid)}
                    _ => panic!("Malformed PSF File (Invalid UTF8 at {:#X})", offset)
                }
                ...
                State::TwoBytesOutofThree(b1,b2) => match byte {
                    0b1000_0000..=0b1011_1111 => {state = State::ZeroBytes; map_char(((b1&0x0F) as u32)<<12|((b2&0x3F) as u32)<<6|((byte&0x3F) as u32), gid)}
                    _ => panic!("Malformed PSF File (Invalid UTF8 at {:#X})", offset)
                }
                ...
                State::ThreeBytesOutofFour(b1,b2,b3) => match byte {
                    0b1000_0000..=0b1011_1111 => {state = State::ZeroBytes; map_char(((b1&0x07) as u32)<<18|((b2&0x3F) as u32)<<12|((b3&0x3F) as u32)<<6|((byte&0x3F) as u32), gid)}
                    _ => panic!("Malformed PSF File (Invalid UTF8 at {:#X})", offset)
                }
                State::Seq => if byte == 0xFF {
    ...
    pub fn get_glyph_bitmap(&self, gid: usize) -> Result<(usize, &[u8]), &'static str> {
        ...
        Ok(
            (self.charsize() / self.height(), &self.psf[offset..offset+self.charsize()])
        )
    }
}
