...
impl<'a> acpi::AcpiHandler for Handler<'a> {
    unsafe fn map_physical_region<T>(&self, physical_address: usize, size: usize) -> acpi::PhysicalMapping<Self, T> {
        use x86_64::structures::paging::{Mapper, PhysFrame, Page, PageTableFlags, Size4KiB};
        ...
        let end_frame = PhysFrame::containing_address(
            PhysAddr::new((physical_address+size) as u64)
        );
        ...
        for frame in PhysFrame::range_inclusive(start_frame, end_frame) {
            let page = Page::from_start_address(
                VirtAddr::new((ACPI_PAGE_START + 4096 * ACPI_PAGE_COUNT.fetch_add(1, Ordering::Relaxed)) as u64)
            ).unwrap();
        ...
        acpi::PhysicalMapping {
            ...
            region_length: size,
            mapped_length: size,
            virtual_start: core::ptr::NonNull::new_unchecked(ptr)
        }
    }

    fn unmap_physical_region<T>(&self, _region: &acpi::PhysicalMapping<Self, T>) {
        // This leaks. Fix this.
    }
}
...
pub unsafe fn init<'a>(
    ...
    let handler = Handler {mapper, frame_allocator};
