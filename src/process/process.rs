use alloc::vec::Vec;
use x86_64::structures::paging::{PhysFrame, Page, FrameAllocator, PageTableFlags, Mapper};
use x86_64::VirtAddr;
use super::Registers;
use goblin::elf64::{
    header::{Header, SIZEOF_EHDR},
    program_header::{ProgramHeader, SIZEOF_PHDR, PT_LOAD},
};
...
pub fn create_process<'a>(paging_service: &mut crate::memory::PagingService, code: &[u8], rip: VirtAddr) -> Process {
    ...
    for code_chunk in code.chunks(4096) {
        let code_frame = paging_service.frame_allocator.allocate_frame().expect("Out of memory");
        const HEAP_END: VirtAddr = VirtAddr::new_truncate((crate::allocator::HEAP_START + crate::allocator::HEAP_SIZE) as u64);
        let copy_page = Page::from_start_address(HEAP_END+4096u64).unwrap();
        let flags = PageTableFlags::PRESENT | PageTableFlags::WRITABLE | PageTableFlags::USER_ACCESSIBLE;
        unsafe {
            paging_service.mapper.map_to(
                copy_page,
                code_frame,
                flags,
    ...
    let user_stack = {
        ...
        for _ in 0..STACK_FRAMES {
            v.push(paging_service.frame_allocator.allocate_frame().expect("Out of memory"));
        }
        v
    };
    ...
    Process {
        ...
        registers: Registers {
            ...
            rsp: super::STACK_TOP.as_u64() as usize,
        }
    }
}

pub fn create_process_from_elf(paging_service: &mut crate::memory::PagingService, data: &[u8]) -> Result<Process, alloc::string::String> {
    let header: Header = unsafe {
        // Safety: There is no invalid state of `Header`, and the `try_into` will make sure the array
        // is correctly sized.
        core::mem::transmute_copy::<[u8;SIZEOF_EHDR], Header>(&data[..SIZEOF_EHDR].try_into().unwrap())
    };
    ...
    let program_headers = {
        ...
        for i in 0..header.e_phnum as usize {
            ...
            let entry = unsafe { core::mem::transmute::<[u8;SIZEOF_PHDR], ProgramHeader>(data[offset..offset+SIZEOF_PHDR].try_into().unwrap()) };
            v.push(entry);
        }
        v
    };
    ...
    for pheader in program_headers {
        if pheader.p_type == PT_LOAD {
            if pheader.p_vaddr != super::PROCESS_START.as_u64() {
                return Err("ELF64 Format Error: Invalid p_vaddr".into());
            }
            let program_data = &data[pheader.p_offset as usize .. pheader.p_offset as usize+pheader.p_filesz as usize];
            return Ok(create_process(paging_service, program_data, super::PROCESS_START));
        }
    }
